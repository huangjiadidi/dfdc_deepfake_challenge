...
from training.datasets.classifier_dataset_kn import DeepFakeClassifierDataset_kn
...
def create_train_transforms_compressed(size=300):
    return Compose([
...
extracted_teacher_feature_maps = []
...
def teacher_hook_function(model, input, output):
    extracted_teacher_feature_maps.append(output)
...
def extract_each_block_landmark(block, landmarks, orginal_image_size=380):
    count = 0
...
def main():
    ...
    teacher_model = classifiers.__dict__[conf['network']](encoder=conf['encoder'])
    ...
    data_train = DeepFakeClassifierDataset_kn(mode="train",
    ...
    data_train_student = DeepFakeClassifierDataset_kn(mode="train",
                                           oversample_real=not args.no_oversample,
                                           fold=args.fold,
                                           padding_part=args.padding_part,
                                           hardcore=not args.no_hardcore,
                                           ...
                                           transforms=create_train_transforms(conf["size"]),
                                           normalize=conf.get("normalize", None))
    data_val_student = DeepFakeClassifierDataset_kn(mode="val",
                                         fold=args.fold,
                                         padding_part=args.padding_part,
                                         crops_dir=args.crops_dir,
                                         data_path=args.data_dir,
    ...
    val_data_loader_student = DataLoader(data_val_student, batch_size=batch_size * 2, num_workers=args.workers, shuffle=False,
                                 pin_memory=False)
    ...
    if conf['fp16']:
        ...
        teacher_model, _ = amp.initialize(teacher_model, teacher_optimizer,
                                          opt_level=args.opt_level,
                                          loss_scale='dynamic')
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        train_data_loader_student = DataLoader(data_train_student, batch_size=batch_size, num_workers=args.workers,
                                       shuffle=train_sampler is None, sampler=train_sampler, pin_memory=False,
                                       drop_last=True)

        train_epoch(current_epoch, loss_functions, model, teacher_model, optimizer, scheduler, train_data_loader, train_data_loader_student,summary_writer, conf,
        ...
        if args.local_rank == 0:
            ...
            if (epoch + 1) % args.test_every == 0:
                bce_best = evaluate_val(args, val_data_loader_student, bce_best, model,
...
def correaltion_loss(landmarks):
    teacher_cropped_maps = []
    cropped_maps = []
    for block in extracted_teacher_feature_maps:
        teacher_cropped_maps.append(extract_each_block_landmark(block, landmarks))

    for block in extracted_feature_maps:
        cropped_maps.append(extract_each_block_landmark(block, landmarks))

    inter_result = frames_inter_loss(cropped_maps[0], teacher_cropped_maps[0])
    intra_result = frames_intra_loss(cropped_maps[0], teacher_cropped_maps[0])
    for i in range(1, len(teacher_cropped_maps)):
        inter_result += frames_inter_loss(cropped_maps[i], teacher_cropped_maps[i])
        intra_result += frames_intra_loss(cropped_maps[i], teacher_cropped_maps[i])
...
def train_epoch(current_epoch, loss_functions, model, teacher_model, optimizer, scheduler, train_data_loader, train_data_loader_student, summary_writer, conf,
    ...
    for i, sample in pbar:
        imgs = sample["image"].cuda()
        labels = sample["labels"].cuda().float()
        landmarks = sample["landmark"]

        teacher_out_labels = teacher_model(imgs)
        out_labels = model(imgs)

        c_loss = correaltion_loss(landmarks)
        if only_valid:
        ...
        loss = (fake_loss + real_loss) / 2 + 0.5 * c_loss
        ...
        c_losses.update(0 if c_loss == 0 else c_loss.item(), imgs.size(0))
        optimizer.zero_grad()
        pbar.set_postfix({"lr": float(scheduler.get_lr()[-1]), "epoch": current_epoch, "loss": losses.avg,
                          "fake_loss": fake_losses.avg, "real_loss": real_losses.avg, "c_loss": c_losses.avg})
